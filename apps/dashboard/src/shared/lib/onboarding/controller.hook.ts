  ...
  const createStoreMutation = useMutation({
    ...
    onSuccess: async (data) => {
      ...
      queryClient.setQueryData(
        appQueries.store.all().queryKey,
        (old: StoreSimpleOutput[] | undefined) => {
          const existing = old ?? [];
