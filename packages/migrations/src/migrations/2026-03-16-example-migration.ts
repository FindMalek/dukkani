 ...
 */
export class ExampleMigrationMigration extends BaseMigration {
	...
	async validatePrerequisites(): Promise<void> {
		...
		logger.info("Validating prerequisites for example-migration");
	...
	async execute(): Promise<MigrationResult> {
		...
		logger.info("Starting database migration: example-migration");
	...
	async rollback(): Promise<MigrationResult> {
		const startTime = Date.now();
		logger.info("Rolling back database migration: example-migration");
