 ...
 * ${DESCRIPTION}
 */
export class ${CLASS_NAME} extends StorageMigration {
	/**
	 * Get migration name
	 */
	getName(): string {
	...
	protected async discoverDatabaseReferencedFiles(): Promise<StorageFileMapping[]> {
		logger.info("Discovering database-referenced files for ${MIGRATION_NAME}");
	...
	protected async discoverAllBucketFiles(): Promise<StorageFileMapping[]> {
		logger.info("Discovering all bucket files for ${MIGRATION_NAME}");
	...
	protected async discoverConfigurableFiles(): Promise<StorageFileMapping[]> {
		logger.info("Discovering configurable files for ${MIGRATION_NAME}");
	...
	protected async uploadBatch(batch: StorageFileMapping[]): Promise<UploadBatchResult> {
		...
		logger.info(`Processing batch of ${batch.length} files for ${MIGRATION_NAME}`);
	...
	protected async updateDatabaseUrls(): Promise<void> {
		if (this.isDryRun()) {
			logger.info("[DRY RUN] Would update database URLs for ${MIGRATION_NAME}");
			return;
		}

		logger.info("Updating database URLs for ${MIGRATION_NAME}");
	...
	protected async cleanupSourceFiles(): Promise<void> {
		if (this.isDryRun()) {
			logger.info("[DRY RUN] Would clean up source files for ${MIGRATION_NAME}");
			return;
		}
		...
		logger.info("Cleaning up source files for ${MIGRATION_NAME}");
	...
	protected async validateResults(): Promise<void> {
		logger.info("Validating migration results for ${MIGRATION_NAME}");
