 ...
 *
 * Run with --dry-run to list objects and plan DB updates without copying.
 */
...
async function main() {
	...
	if (
		...
		console.error(
			"Destination: Set S3_ENDPOINT, S3_ACCESS_KEY_ID, S3_SECRET_ACCESS_KEY, S3_BUCKET, S3_PUBLIC_BASE_URL",
		);
		process.exit(1);
	}

	const supabaseProject = new URL(SUPABASE_URL).hostname.replace(
		".supabase.co",
		"",
	);
	const supabaseBase = `https://${supabaseProject}.supabase.co/storage/v1/object/public/${SUPABASE_BUCKET}/`;
	const r2Base = S3_PUBLIC_BASE_URL.endsWith("/")
		? S3_PUBLIC_BASE_URL
		: S3_PUBLIC_BASE_URL + "/";

	console.log("Source:", supabaseBase);
	console.log("Destination:", r2Base);
	console.log("");
