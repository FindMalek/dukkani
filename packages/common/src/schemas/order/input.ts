...
/** Plain object shape; use for `.omit()` / `.extend()` before applying refinements. */
export const createOrderPublicInputObjectSchema = z.object({
  ...
  customerPhone: z
    ...
    .refine(
      (value) =>
        /^[\d\s+\-()]+$/.test(value) && (value.match(/\d/g) || []).length >= 8,
...
export const createOrderPublicInputSchema =
  createOrderPublicInputObjectSchema.refine(
